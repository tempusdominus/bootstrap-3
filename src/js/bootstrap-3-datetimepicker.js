...
const Bootstrap3DateTimePicker = ($ => {
    ...
    class Bootstrap3DateTimePicker extends DateTimePicker {
        ...
        _fillMinutes() {
            const table = this.widget.find('.timepicker-minutes table'),
                  ...
                  html = [];
            let row = $('<tr>');
            const step = this._options.stepping === 1 ? 5 : this._options.stepping;
        ...
        _doAction(e, action) {
            ...
            switch (action) {
                ...
                case "decrementMinutes":
                    {
                        const newDate = this._date.clone().subtract(options.stepping, 'm');
                        if (this._isValid(newDate, 'm')) {
                            this._setValue(newDate);
                        }
                        break;
     ...
     * ------------------------------------------------------------------------
     */
    const getSelectorFromElement = function ($element) {
        ...
        const config = $selector.data(DateTimePicker.DATA_KEY) ? '' : $.extend({}, $selector.data(), $(this).data());
